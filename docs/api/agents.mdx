...
Install `youtube-direct` for direct, no-account stateless access from the user's machine. Install `video2ctx-api` instead for managed, authenticated hosted discovery and extraction, or `video2ctx-monitoring` for the stateful monitor exception. The hosted skills carry a self-contained CLI that can open a browser-based device login and call the production API without an npm package or a secret in the prompt.
