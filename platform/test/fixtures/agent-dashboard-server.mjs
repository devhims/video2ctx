...
createServer(async (req, res) => {
  ...
  if (url.pathname.startsWith('/v1/agent')) {
    ...
    if (url.pathname === '/v1/agent' && req.method === 'POST') {
      ...
      const receipt = { sessionId: id, runId, agentMessageId: crypto.randomUUID(), status: 'pending',
    ...
    if (eventPath) {
      ...
      timers.push(setTimeout(() => send({ run: { sessionId: id, runId, status: 'running' }, phase: 'finalization',
        draft: { answer: 'The agent is assembling the evidence-backed comparison.', state: 'streaming' }, tools: [finishedTool] }), 600));
    ...
    if (detailId) {
      ...
      if (latestTurn && !older) {
        ...
        return reply(200, { ...current, lastRunId: receipt.runId, messages: [message('user', 2, detailId), message('assistant', 2, detailId),
          ...
          { ...message('assistant', receipt.diagnostics.conversationTurn, receipt.runId, completed ? 'completed' : 'running'), messageId: receipt.agentMessageId }], nextCursor: null });
      }
