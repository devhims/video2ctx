...
export const OPENAPI_OPERATION_AUDIENCE: Readonly<Record<string, OpenApiAudience>> = {
  ...
  startDeviceAuthorization: 'consumer',
  exchangeDeviceAuthorization: 'consumer',
  verifyDeviceAuthorization: 'first-party',
  approveDeviceAuthorization: 'first-party',
  denyDeviceAuthorization: 'first-party',
  getAuthSession: 'first-party',
  signOut: 'first-party',
  ...
  listAgentSessionAssets: 'consumer',
  getAgentSessionAsset: 'consumer',
  deleteAgentSessionAssets: 'consumer',
  deleteAgentSessionAsset: 'consumer',
  deleteAgentSessionMemory: 'consumer',
  ...
  getBilling: 'first-party',
  getUsage: 'consumer',
  getAdminAccess: 'operator',
  listAgentAccess: 'operator',
...
export const OPENAPI_INTERNAL_SAFETY: Readonly<Record<string, string>> = {
  ...
  verifyDeviceAuthorization: 'Claims a short-lived user code for the signed-in browser account; show the account and requested access before a decision.',
  approveDeviceAuthorization: 'Issues CLI access after an explicit browser decision; approve only a code the user initiated on a device they control.',
  denyDeviceAuthorization: 'Terminates the pending device request selected by the signed-in user without issuing a credential.',
  getAuthSession: 'Returns the current credential identity; never include raw cookie or bearer material in the response or logs.',
  signOut: 'Revokes the presented session; invoke it only for an explicit logout action and clear the matching local credential after success.',
  ...
  getBilling: 'Returns subscription state for the current browser account; do not expose another customer’s provider identifiers.',
  ...
  getAdminAccess: 'Checks current operator authorization using a live browser session; does not grant Agent access.',
  listAgentAccess: 'Exposes tester email addresses; restrict to authorized operators.',
