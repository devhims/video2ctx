...
export class VideoCatalog {
  ...
  async read<T>(key: VideoAssetKey): Promise<StoredVideoAsset<T> | null> {
    ...
    try {
      const value = await this.hydrate(JSON.parse(payload) as Json, key.videoId);
      return {
        value: value as T,
  ...
  async save(
    ...
    try {
      await this.write(key, value, fetchedAt, maxAgeMs, complete, coverage);
    } catch (cause) {
  ...
  private async write(
    ...
    await this.db
      ...
      (video_id,kind,variant,content_hash,object_key,bytes,fetched_at,fresh_until,complete,coverage_json,state)
      VALUES (?,?,?,?,?,?,?,?,?,?,'pending')
      ON CONFLICT(video_id,kind,variant,content_hash) DO UPDATE SET
      fetched_at=MAX(fetched_at,excluded.fetched_at), fresh_until=MAX(fresh_until,excluded.fresh_until), state='pending'`,
      )
      .bind(
        key.videoId,
        key.kind,
  ...
  private async publish(row: AssetRow): Promise<void> {
    await this.db.batch([
      ...
      this.db
        ...
        WHERE video_id=? AND kind=? AND variant=? AND content_hash=? AND fetched_at<=?`,
        )
  ...
  private async dehydrate(value: Json, videoId: string, images: ImageWrite[]): Promise<Json> {
    ...
    for (const [name, item] of Object.entries(value)) {
      if (name === 'imageBase64' && typeof item === 'string') {
        ...
        const key = `youtube/videos/${videoId}/images/${hash}.jpg`;
