...
function serialize(format: Format, project: { name: string; description: string }, items: ItemRow[]): string {
  ...
  if (format === 'csv') {
    const lines = [['provider','title','type','entity_id','start_ms','end_ms','note','tags'].join(',')];
    for (const item of items) lines.push([item.provider,item.title,item.entity_type,item.entity_id,item.start_ms,item.end_ms,item.note,item.tags_json].map(csv).join(','));
    return lines.join('\n');
  }
  ...
  for (const item of items) {
    const url = item.provider === 'youtube' && item.entity_type === 'video'
