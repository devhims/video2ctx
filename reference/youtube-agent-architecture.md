...
- [Capability routing](youtube-agent-capability-routing.html): the first semantic step that turns one prompt into a `topic_research` configuration, an `inspect_video` configuration, or a direct `finalize` decision.
...
`POST /v1/agent` authenticates the caller, establishes or reuses a session, checks the current credit balance, and invokes `AgentRuntimeDO.startRun()`. New conversations receive separate Durable Object identities. Follow-up calls route back to the same user-scoped conversation object. Admission assigns and returns a stable `userMessageId`, `agentMessageId`, and `conversationTurn` with the run receipt. The receipt also exposes `modelStepCount` for completed Agent Core model steps and `toolCallCount` for persisted tool calls. After admission, the route records a bounded session projection in the authenticated user's `UserAccountDO`. `GET /v1/agent/sessions` lists that user's sessions by recent activity and optionally performs lexical FTS5 search over recorded user prompts. Opaque cursors provide pagination. This catalog does not use embeddings, semantic retrieval, or model context.
...
The application owns the phase limits: up to 40 seconds of research after classification, including provider fetches and transcript analysis, followed by up to 60 seconds of finalization. Classification has its own 20-second timeout. Saving has a separate 30-second persistence timeout. The application also enforces eight nominal Agent Core steps, twelve total tool calls, four concurrent provider operations, typed evidence packets, route and final-intent validation, citation validation, run-ID-based queue delivery deduplication, and durable checkpoints.
...
Conversation memory does not use D1 or `UserAccountDO`. Each completed `agent_runs` row is one durable user-assistant turn. Its `turn_ordinal` records admission order, and its `parent_message_id` points to the prior assistant message so branches remain isolated and recovery reconstructs the same ancestor chain. Agent model calls, including classification, research, finalization, repair, and source analysis, receive at most the eight most recent completed ancestors, with no history character limit. User messages and assistant answers are replayed as conversation history. Direct routing and finalization separately receive persisted source packets cited by those ancestors; raw tool traces are not replayed. The restoration endpoint applies the same separation when producing UI messages. Agent Runtime tables also store route decisions, tool calls, evidence packets, and events in Durable Object SQLite. The Agents SDK stores fiber checkpoints in its own ledger in the same conversation object.
...
Classification also gates scope and storyboard access. New decisions select `topic_research`, `inspect_video`, or `finalize`. A `finalize` route carries `responseIntent: context_answer | clarification | rejected` and a routing reason. The shared finalizer generates the response with history and available evidence, without constructing the research loop or fetching evidence. Legacy persisted `clarification` and `rejected` routes also use this finalizer. A declined request uses `intent: rejected` in legacy format or `outcome: rejected` in compact format, with an `OUT_OF_SCOPE` warning. New executable routes require `useStoryboard`; false removes the visual tool from the model and disables provider access. True makes sampled visual inspection available within the same research budget. Legacy persisted routes without the flag retain their original tool set.

Classification, research, and finalization are separate phases. Classification has a 20-second timeout; research starts its 40-second clock only after routing completes. Finalization starts its own 60-second clock at handoff, including when research finishes early. Each deadline is saved in Durable Object SQLite and reused on recovery. Recovery during finalization skips research and uses the remaining finalization time. Abort signals cancel phase work, and a deadline race bounds non-cooperative calls. User cancellation and terminal-state checks still apply. Saving a validated answer has a separate 30-second timeout, and billing settlement is retried durably if needed. The three model phases total at most 120 seconds, but queueing and persistence can extend end-to-end completion.
...
## Unified finalization and caching

Direct context answers, clarifications, declines, and researched answers use the same generation, repair, citation validation, deadline, and persistence path. Research answers require evidence references. Context-answer blocks may omit references only when discussing the conversation itself. Clarification and rejection schemas prohibit references. Source references are validated against persisted evidence on the selected branch.

The finalizer sends stable instructions followed by chronological history, the current request, routing intent, evidence, and optional repair feedback. Repair feedback does not change the system prompt. Fireworks finalizers use the same session-affinity cache key as other model calls. Actual reuse is measured through persisted cached-input token counts; a shared key does not guarantee cache hits or share caches across different models.
