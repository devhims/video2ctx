...
Research now records every pending tool as interrupted before taking the finalizer snapshot. The finalizer therefore receives both the storyboard failure and the interrupted frame call, even when the provider takes time to acknowledge cancellation. Regression tests reproduce the deadline mismatch and delayed-cancellation reporting race with fake time, then verify successful analysis after a 45-second retrieval, interrupted-tool warnings, the production visual window, and preservation of saved deadlines.

## Bounded frame completion

The retry in run `7eaee2aa-f8a1-4d94-b682-26ec8766de53` returned five frames in its first batch, but a second batch exhausted the research window. It also missed legible names in broadcast graphics because its analysis focus had been narrowed to jersey backs.

Frame calls now reserve 30 seconds of remaining research time: 20 seconds for vision, 5 seconds for transport/cleanup, and 5 seconds for preview saving and handoff. Extraction receives the lesser of 45 seconds and the remaining research time minus that reserve. When fewer than 5 extraction seconds remain, the tool is removed from the next model step and a late call is rejected before contacting the container. This does not increase the current 120-second visual research budget or restore a strict 100-second end-to-end limit.
...
`ffmpeg_success` records the winning client profile, candidate index, format ID when present, requested timestamp, output dimensions, source dimensions and elapsed time. This identifies which fallback produced a lower-resolution image without retaining its signed URL.
