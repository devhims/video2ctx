...
- Optional server-side environment override: `PLATFORM_API_BASE_URL=https://api.video2ctx.dev`
- Optional browser-side environment override: `NEXT_PUBLIC_PLATFORM_API_BASE_URL=https://api.video2ctx.dev`
...
`npm run verify` installs the processor dependencies from its lockfile, including the published `all-things-youtube` version. Library releases must be published to npm before updating that dependency. GitHub CI and Cloudflare Builds use the same verification command; neither needs locally compiled library artifacts for the processor.
...
The Redis quota allows five distinct YouTube video IDs per HMAC-hashed visitor IP in a rolling 24-hour window. Repeating a video does not consume another slot.
