...
The checked-in production configuration uses GLM 5.3 Flash for classification, Agent Core, transcript analysis, storyboards, and frames. Updated DeepSeek V4.1 Flash handles final answers. Both optional text-profile settings are blank, so research uses the established GLM provider and low-reasoning settings. Setting `AGENT_TEXT_PROVIDER=fireworks` and `AGENT_TEXT_MODEL=deepseek-v4p1-flash` opts text roles into DeepSeek with reasoning disabled; visuals continue using GLM. Setting a text provider without a model fails explicitly. Fireworks is the default GLM provider, using accounts/fireworks/models/glm-5p3-flash. `AGENT_GLM_PROVIDER=workers-ai` restores the retained Workers AI research provider and configured AI Gateway using @cf/zai-org/glm-5.3-flash. `AGENT_FINALIZER_PROVIDER=fireworks` independently selects the native Fireworks AI SDK provider for finalization and terminal argument repair. It requires the `FIREWORKS_API_KEY` Worker secret and fails explicitly if configuration is missing or unsupported. Configuration describes the intended rollout; successful production deployment must be verified separately.

All Fireworks agent requests set `service_tier: priority`, including classification, research, analysts, repairs, and finalization. Priority reduces overload rejection risk; it does not guarantee completion within the existing phase deadlines. Cost accounting uses Priority rates per million tokens: GLM 5.3 Flash $0.1875 input / $0.0375 cached input / $0.625 output; DeepSeek V4.1 Flash $0.375 / $0.0075 / $1.50; retained DeepSeek V4 Flash 0731 $0.275 / $0.00875 / $0.825; GPT-OSS 120B $0.18 / $0.018 / $0.72. Workers AI pricing is unchanged. Source: https://docs.fireworks.ai/serverless/pricing.

`AGENT_FINALIZER_MODEL` selects `glm-5p3-flash` (default), `deepseek-v4p1-flash`, the retained `deepseek-v4-flash-0731` profile, or `gpt-oss-120b`. Production config selects `deepseek-v4p1-flash` with a blank `AGENT_FINALIZER_REASONING_EFFORT`, retaining the historical 1,024-token thinking allowance. For optional GLM comparisons, `medium` maps explicitly to Fireworks `high`, and `low` is also supported. This effort setting applies to GLM finalizer calls, including stored-context gathering, not to research or visual analysts. GLM does not expose a literal medium setting. Unknown effort values and using this effort override with another model fail explicitly. Clear the override to use a retained model profile: GLM and DeepSeek then use the historical 1,024-token thinking allowance; GPT-OSS uses native low effort. Optional DeepSeek research disables reasoning and preserves its original answer allowance. Finalizers retain 1,024 tokens of output headroom because Fireworks includes reasoning in `max_tokens`; this is a combined ceiling, not a guaranteed reservation. Completed usage is recorded against actual model IDs and configured prices, including cached input. Research drafts are not persisted as final answers. Model selection does not change phase deadlines.
...
The visual call has a 20-second timeout, no automatic model retry, and shares the collection phase cancellation signal. The checked-in production configuration uses GLM 5.3 Flash for classification, planning, and transcript analysis. Storyboard and frame analysis use GLM 5.3 Flash with low reasoning. The planner receives text findings without image bytes. The configured GLM finalizer uses structured output and medium effort, mapped to the provider's high setting. Fireworks requires FIREWORKS_API_KEY, and usage records retain the actual provider model ID and its configured pricing. Text research keeps its 40-second deadline; visual research has a shared 120-second window. The standalone public endscreen and trends API routes are unaffected.
...
The finalizer's transmitted schema includes `needsEvidence` only when the route permits another inspection. History-only and executable research finalization cannot select that field. Allowed context inspection still validates the selected video against known session references and returns to the same finalizer once.

Transcript quantities require exact source quotes. Equivalent English cardinal numbers, comma grouping, and explicit thousand/million/billion scales are recognized without changing captions. Hours, minutes, seconds, and explicitly named Celsius/Fahrenheit units accept common notation aliases. These checks do not convert values, infer missing temperature units, or reconstruct fragmented decimals. Counts should carry their explicit object unit, such as microphones or voice samples. A missing unit still requires uncertainty.

Transcript analysis permits at most two logical attempts under the existing shared cancellation signal and token ceiling. A repair uses at most three findings, one identity, and three quantities per finding. This also handles incomplete JSON caused by output truncation. Usage is recorded for truncated responses when supplied by the SDK. Unsupported findings are still rejected or reduced to independently validated quantities; the model must not fill a gap with invented measurements. Successful schema validation is not semantic proof that every relevant source fact was retained.
