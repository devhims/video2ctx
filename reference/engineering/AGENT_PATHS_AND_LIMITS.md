...
All Fireworks agent requests set `service_tier: priority`, including classification, research, analysts, repairs, and finalization. Priority reduces overload rejection risk; it does not guarantee completion within the existing phase deadlines. Cost accounting uses Priority rates per million tokens: GLM 5.3 Flash $0.1875 input / $0.0375 cached input / $0.625 output; DeepSeek V4 Flash 0731 $0.275 / $0.00875 / $0.825; GPT-OSS 120B $0.18 / $0.018 / $0.72. Workers AI pricing is unchanged. Source: https://docs.fireworks.ai/serverless/pricing.

The opt-in `platform/scripts/benchmark-fireworks-tiers.ts` compares Standard and Priority through the production SDK with synthetic text and four generated color sheets. It sends 30 paid requests with alternating tier order, separate cache-affinity keys, no retries, and a 40-second observation limit. It records end-to-end timing, HTTP status, provider timing headers, usage, and output validation. Run from `platform/` with `node --env-file=.dev.vars --import tsx scripts/benchmark-fireworks-tiers.ts /tmp/fireworks-tiers.json`. Five samples per tier and workload are a smoke comparison, not a peak-load reliability test.
