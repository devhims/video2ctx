...
`AGENT_FINALIZER_MODEL` selects `glm-5p3-flash` (default), `deepseek-v4p1-flash`, the retained `deepseek-v4-flash-0731` profile, or `gpt-oss-120b`. Production selects `deepseek-v4p1-flash`, mapped to `accounts/fireworks/models/deepseek-v4p1-flash`. The older profile remains for compatibility and historical pricing; its availability is not guaranteed. Classification, planning, and transcript extraction disable native reasoning to preserve their output allowances and the 40-second text research deadline. The finalizer retains native thinking with a 1,024-token budget. Visual GLM requests retain low reasoning. GPT-OSS uses native `reasoningEffort: low`, which does not impose a separate reasoning-token cap. The adapter adds 1,024 tokens of reasoning headroom only to finalizer output allowances because Fireworks includes reasoning in `max_tokens`. This is a combined ceiling, not a guaranteed reservation of all requested answer tokens. Completed usage is recorded against the returned model and its configured prices, including cached input. When a separate finalizer is configured, ordinary completion and malformed terminal arguments also hand off to it. GLM research drafts are not persisted as final answers. Changing the provider does not change any phase deadline. Available configuration is not evidence that a model meets a production reliability target.
