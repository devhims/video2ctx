...
- `AgentSessionProvider` stores a searchable projection of user messages and completed answers. `Session.search()` searches its SQLite FTS index. History search in this pinned version matches a literal phrase. A paginated reader returns 20 messages per page because keyword search cannot enumerate every message reliably. The finalizer retains its five-step and cost budgets, reserving the last step for an answer; it must state when pagination is incomplete. Results cover the session and can include other conversation branches; they are not asserted to be the active branch alone.
