...
6. Citation validation resolves immutable excerpt IDs against persisted packets. Validated memory updates are committed in SQLite with the completed answer. Findings require existing evidence references; context and open questions are stored separately. Matching kind/topic replaces an earlier memory, allowing user corrections.
...
Frame tool results expose `sessionReused` in the activity trace when all requested images came from session storage. The dashboard labels this as analysis of saved frames. A new visual focus can still invoke an analyst; it does not imply another extraction or retrieval charge.
