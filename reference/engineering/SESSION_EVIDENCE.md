...
2. Classification chooses research, single-video inspection or direct finalization. Specific-video comparisons carry `comparisonVideoIds` independently of the retrieval route. With one missing transcript, inspection fetches that video and preserves every comparison subject; with several missing transcripts, research uses those subjects without discovery. A narrow guard repairs the previously failing "compare this video with a new one" form if the earlier subject is dropped. Other comparison phrasing is resolved by classification. Context answers carry a history/video/mixed scope; exact-first-message and all-user-message requests also carry an explicit history selection. Persisted older routes remain readable. Explicit requests to fetch again set `refreshEvidence` and require an executable route.
3. Retrieval checks the session store before the provider. Complete nonempty transcripts are keyed by video and requested/resolved language; comments by video and continuation; frames by video, timestamp and width; storyboard sheets by video, manifest version and sheet index. Raw images and transcripts are persisted by retrieval tools that never invoke an analyst. For single-video inspection, the main model reads the returned timed captions directly. Research selects sources and a focus once using `research_video_transcripts`. Its application scheduler retrieves missing transcripts or reuses selected saved versions, then starts each analysis as soon as that transcript is saved. Retrieval and analysis have separate concurrency limits, so slow provider requests cannot occupy ready analysis slots. Each child retrieval and analysis persists independently; completed evidence remains available to finalization when a sibling fails or the research deadline cancels unfinished work. Saved-only follow-up analysis can use `analyze_video_transcripts`; visual interpretation uses `analyze_video_frames` or `analyze_video_storyboard` with saved frame or sheet versions and a focus. These analysis tools load exact session payloads, never call YouTube, and persist derived evidence separately. A new question can reuse the same raw assets without a retrieval tool call. A refresh advances compatible language aliases while preserving older versions for historical citations.
4. Research, inspection and finalization can use the SDK-generated `search_context` tool for `history`, `memory`, and `evidence`. `read_session_history` lists original messages chronologically with role filtering and pagination, including messages beyond the eight-turn prompt window. Stored payloads are not automatically injected into every run. Finalization can read saved transcript pages or existing analysis through `read_session_evidence`. It can list more inventory when the prompt omitted assets. Inventory and memory are hints, not evidence for factual claims. Finalization cannot request another inspection or fetch new provider evidence. Routing selects research or inspection when fresh evidence is needed. If stored context is insufficient after finalization begins, the answer states that gap. Research and inspection end in the same finalizer.
5. The unified finalizer first gathers context without an answer JSON schema, then generates structured answer blocks in a separate call with tools disabled. History requests receive the first chronological page before model inference. Exact-first-message answers must contain that original text verbatim. Finalization has one persisted 60-second deadline. Context gathering is limited to four model steps and 10 seconds; answer generation reserves up to 20 seconds for one repair within that deadline and the existing cost limit. Standard/detailed structured answers allow 3,000/4,000 output tokens, with another 1,000 for repair, plus provider reasoning headroom. Comparison finalization preloads the current saved transcript for every subject in parallel, without provider calls, capped at 5,000 excerpts per subject. Full reads retain the same citation IDs as paged reads. Short citation aliases are assigned before measuring the prompt budget; comparisons divide a 160,000-character evidence allowance across subjects. If a transcript exceeds its allowance, the projection retains as many sampled passages across its timeline as fit and flags incomplete coverage. Validation requires citations for every subject or an explicit answer-scope shortfall. Known fragment-only and promise-only non-answers trigger repair rather than successful persistence. These narrow checks do not prove semantic completeness for arbitrary answers. Context tool names and finish reasons are logged without message contents.
6. After context gathering, the finalizer schema permits answer fields only, rejects unexpected inspection requests for repair, and restricts answer and memory evidence IDs to supplied excerpt references (short aliases or full IDs). Inventory asset IDs and unrelated history citations are not accepted. Citation validation still resolves immutable excerpt IDs against persisted packets. Validated memory updates are committed in SQLite with the completed answer. Findings require existing evidence references; context and open questions are stored separately. Matching kind/topic replaces an earlier memory, allowing user corrections.
...
Deleting an asset removes its aliases, derived packets, dependent findings, historical source excerpts and image previews. Historical messages remain; affected answers have unavailable source markers and are omitted from future assistant context. Deleting all assets also removes all memory. Deletion increments a generation before asynchronous cleanup. Older retrievals and memory snapshots cannot restore deleted state. Explicit session deletion uses a durable queue for private R2 cleanup and retries when the object starts again. Shared catalog references are removed locally, but session and account deletion never delete shared D1 versions or `VIDEO_ASSETS` objects. A missing shared version fails closed; a newer catalog version cannot replace cited historical evidence.

Table answers may place `[cite:ref_N]` markers inside Source cells, provided those references are declared in the same block's evidence IDs. The application resolves aliases, validates persisted evidence, and preserves their cell positions. References not placed inline appear below the table rather than creating a trailing cell. Compact responses group excerpts by source video and show source numbers. The dashboard links those numbers to the corresponding safe source URL and exposes the source title on hover. Older escaped `(source marker:...)` text is normalized on compact reads only when it matches an already validated citation. Unmatched markers display `[source unavailable]`; stored answers are not rewritten.
...
Legacy raw session blobs migrate on read, or in bounded batches of ten when their session inventory opens. The shared source is verified before a synchronous SQLite transaction attaches its immutable reference. The original private blob and key remain available until the migration is confirmed; the backfill never queues them for deletion. Failures retain the readable private copy and retry on a later read. Stable session IDs and citation IDs do not change. Historical imports do not advance the public current pointer or freshness. Unsupported payloads with private fields stay in private storage.

An operator can also sweep every dashboard session, including dormant sessions, using the [session asset migration runbook](./SESSION_ASSET_MIGRATION.md). The sweep attaches immutable shared references and verifies reads from those references, retaining every original private blob. Deployment alone does not run the sweep. Historical run snapshots and existing JPEG preview links are outside this raw-asset migration. Existing historical run packets remain readable; old runs are not retroactively converted into raw asset storage. Apply catalog migration `0003_historical_asset_versions.sql` before deploying this code. Session-local tables are created automatically.
